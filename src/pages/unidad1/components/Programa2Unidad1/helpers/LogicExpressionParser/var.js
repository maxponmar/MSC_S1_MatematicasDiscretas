...
import { MissingValue, MissingTruthValue } from "./errors";
...
class LogicVar {
  constructor(
    ...
    if (value == null && nodeObj == null) {
      throw new MissingValue(
        "Required variable for LogicVar. Must use either value or nodeObj."
      );
    }
    ...
    // Check if nodeObj is not null
      ...
      if (!("value" in nodeObj)) {
        throw new MissingValue(
          'Required key for LogicVar. Must have "value" key in nodeObj.'
        );
      } else {
  ...
  evaluate(truthValue = {}) {
    if (!(this.getValue() in truthValue)) {
      throw new MissingTruthValue(
        `Required truth value for the variable "${this.getValue()}".`
      );
    }
