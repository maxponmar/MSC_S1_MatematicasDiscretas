...
function SidebarLink({ title, link, href, onClick }) {
  ...
  if (link)
    sidebarItem = (
      <Link
        to={link ? link : "/"}
        className=" w-full h-12 flex items-center justify-end pr-5 py-5 my-1 ease-in-out duration-300 hover:bg-itn hover:text-white cursor-pointer font-bold text-xl text-right"
        onClick={onClick}
      >
        {title}
      </Link>
